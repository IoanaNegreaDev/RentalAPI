...
namespace RentalAPI.Services
{
        ...
        IContractService
    {
        ...
        public async Task<Contract> FindByIdAsync(string userId, int id)
            => await _repository.FindByIdAsync(userId, id);

        public async Task<DbOperationResponse<IEnumerable<Contract>>> ListAsync(string userId)
        {
            var dbUser = await _userRepository.FindByIdAsync(userId);
            if (dbUser == null)
                return new DbOperationResponse<IEnumerable<Contract>>("Failed to find user.");
            var result = await _repository.ListAsync(dbUser.Id);
            return new DbOperationResponse<IEnumerable<Contract>>(result);
        }
        ...
        public async Task<DbOperationResponse<Contract>> UpdateAsync(string userId, int contractId, int paymentCurrencyId)
        {
            var dbCurrency = await _currencyRepository.FindByIdAsync(paymentCurrencyId);
            if (dbCurrency == null)
                return new DbOperationResponse<Contract>("Currency not supported. Check the supported currencies.");

            var existingContract = await _repository.FindByIdAsync(contractId);
            if (existingContract == null)
                return new DbOperationResponse<Contract>("Contract not found.");

            if (existingContract.User.Id != userId)
                return new DbOperationResponse<Contract>("Invalid contractId for this user.");

            existingContract.PaymentCurrencyId = paymentCurrencyId;
        ...
        public async Task<DbOperationResponse<Contract>> UpdateAsync(int contractId, int paymentCurrencyId)
        {
            var dbCurrency = await _currencyRepository.FindByIdAsync(paymentCurrencyId);
            if (dbCurrency == null)
                return new DbOperationResponse<Contract>("Currency not supported. Check the supported currencies.");
